...
fileprivate let screenPercentageCovered: CGFloat = 0.5
fileprivate let tableRowSize: CGFloat = 80

class FiddleScreenView: UIView, FiddleScreenViewProtocol, UITableViewDelegate, UITableViewDataSource {
    weak var featureLogic: FiddleScreenLogicProtocol!
    ...
    convenience init(_ featureLogic: FeatureLogicProtocol) {
        ...
        guard let logic = featureLogic as? FiddleScreenLogicProtocol else {
    ...
    func initUI() {
        self.frame = CGRect(x: UIScreen.main.bounds.maxX, y: CGFloat(0),
                            width: UIScreen.main.bounds.width * screenPercentageCovered,
                            height: UIScreen.main.bounds.height)
        self.tableView.frame = CGRect(x: CGFloat(0), y: CGFloat(0),
                                       width: UIScreen.main.bounds.width * screenPercentageCovered,
                                       height: UIScreen.main.bounds.height)
        self.tableView.isScrollEnabled = true
        self.tableView.rowHeight = tableRowSize
    ...
    func show(_ onShowing: (() -> Void)?) {
        ...
        UIView.animate(withDuration: 0.5, delay: 0, options: UIView.AnimationOptions.curveEaseOut, animations: {
            self.frame = CGRect(x: self.startX, y: CGFloat(0),
                                width: UIScreen.main.bounds.width * screenPercentageCovered,
                                height: UIScreen.main.bounds.height)
        }, completion: { _ in
            onShowing?()
    ...
    func hide(_ onHidden: (() -> Void)?) {
        UIView.animate(withDuration: 0.5, delay: 0, options: UIView.AnimationOptions.curveEaseOut, animations: {
            self.frame = CGRect(x: UIScreen.main.bounds.maxX, y: CGFloat(0), width: UIScreen.main.bounds.width * screenPercentageCovered, height: UIScreen.main.bounds.height)
        }, completion: { _ in
            self.alpha = 0
            onHidden?()
        })
...
class PropertyCell: UITableViewCell {
    ...
    func setupView() {
        view.frame = CGRect(x: 0, y: 0,
                            width: UIScreen.main.bounds.width * screenPercentageCovered,
                            height: tableRowSize).insetBy(dx: 20, dy: 10)
